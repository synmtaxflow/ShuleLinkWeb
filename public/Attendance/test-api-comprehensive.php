...
/**
 * Comprehensive API Testing Script - All 13 Endpoints
 * Tests all API endpoints with proper validation and error handling
 * 
 * Usage: php test-api-comprehensive.php [base_url]
 * 
 * Example: php test-api-comprehensive.php http://127.0.0.1:8000/api/v1
 */
...
echo "\n";
echo "{$blue}========================================{$reset}\n";
echo "{$blue}  Comprehensive API Testing Script{$reset}\n";
 ...
 * Test an endpoint
 */
function testEndpoint($name, $method, $url, $data = null, $expectedStatus = 200, $headers = []) {
...
$registerResult = testEndpoint(
    ...
    [
        ...
        'auto_register_device' => false // Set to false to avoid device connection issues during testing
    ],
    201
);
...
if ($testUserId) {
    testEndpoint(
        ...
        "{$baseUrl}/users/{$testUserId}",
        [
            'name' => 'Updated Test User ' . date('H:i:s')
        ],
        200
    );
}
...
testEndpoint(
    ...
    [
        ...
        'email' => 'duplicate2' . time() . '@example.com',
        'password' => 'password123',
...
// Test 22: Register User with Missing Required Fields (should fail)
testEndpoint(
    '22. Register User (missing required fields - should fail)',
    'POST',
    "{$baseUrl}/users/register",
    [
        'name' => 'Incomplete Test'
        // Missing email, password, enroll_id
    ],
    422
);
