...
return new class extends Migration
{
    ...
    {
        Schema::table('book_losses', function (Blueprint $table) {
            if (!Schema::hasColumn('book_losses', 'payment_status')) {
                $table->enum('payment_status', ['unpaid', 'paid'])->default('unpaid')->after('status');
            }
            if (!Schema::hasColumn('book_losses', 'payment_method')) {
                $table->enum('payment_method', ['replace', 'cash'])->nullable()->after('payment_status');
            }
            if (!Schema::hasColumn('book_losses', 'payment_amount')) {
